...
@student_lesson_quiz_bp.route('/student-lesson-quizzes/<int:record_id>', methods=['GET'])
@jwt_required()
@response_wrapper
def get_student_lesson_quiz_record(record_id):
    """
    GET /student-lesson-quizzes/<record_id>

    Description:
    Get a single student lesson quiz record by ID.

    Path Parameters:
    - record_id: int — The ID of the student lesson quiz record to retrieve.

    Returns:
    - 200: JSON object of the student lesson quiz record (marshmallow schema)
    - 404: If student lesson quiz record not found
    """
